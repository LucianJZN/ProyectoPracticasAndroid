...
import com.example.proyectopracticasandroid.BaseActivity
import com.example.proyectopracticasandroid.LoginUserActivity
import com.example.proyectopracticasandroid.R

class AdminProductActivity : BaseActivity() { // Hereda de BaseActivity
        ...
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    Toast.makeText(this@AdminProductActivity, "Producto creado correctamente", Toast.LENGTH_SHORT).show()
                    ...
                    Toast.makeText(this@AdminProductActivity, "Acceso denegado. Por favor, inicie sesión de nuevo.", Toast.LENGTH_LONG).show()
                    tokenStorage.deleteAuthToken() // Elimina el token inválido
                    startActivity(Intent(this@AdminProductActivity, LoginUserActivity::class.java))
                    finish()
                    dialog.dismiss()
                }
                else {
                    ...
                    Toast.makeText(this@AdminProductActivity, "Error al crear producto", Toast.LENGTH_SHORT).show()
                ...
                Toast.makeText(this@AdminProductActivity, getString(R.string.error_api_connection), Toast.LENGTH_SHORT).show()
            }
        }
    }
                ...
                if (response.isSuccessful) {
                    ...
                    if (productos != null) {
                        ...
                        adapterProduct = ProductAdapter(
                            ...
                            this@AdminProductActivity     // Contexto de la Activity
                        )
                        ...
                        Toast.makeText(this@AdminProductActivity, "Error: Datos de productos vacíos.", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    val errorBody = response.errorBody()?.string()
                    val errorCode = response.code()
                    Log.e("ProductActivity", "Error API al cargar productos: Código $errorCode, Contenido: $errorBody")
                    Toast.makeText(this@AdminProductActivity, "Error al cargar los productos (${errorCode})", Toast.LENGTH_SHORT).show()
                }
            }
            override fun onFailure(call: Call<List<Product>>, t: Throwable) {
                Log.e("ProductActivity", "Fallo de conexión al obtener productos: ${t.message}", t)
                Toast.makeText(this@AdminProductActivity, "Error de conexión", Toast.LENGTH_SHORT).show()
            }
        })
    }
}